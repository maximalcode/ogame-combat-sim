use combat_ogame_api::reports::{
    CompletionEvidence, CompletionInput, CompletionResult, EvidenceSource, PinnedUniverse,
    PinnedUniverseSettings, TechnologyBasis, TechnologyCandidate, TechnologyEvidence,
    complete_candidate,
};
...
mod support;
use support::{candidate, evidence, participant, universe};
...
fn completing_a_private_observation_does_not_bypass_fixture_publication_consent() {
    ...
    let comparison = combat_ogame_api::reports::compare_with(&input, |request| {
        combat_core::Simulator::new()
            .simulate_multiple(request)
            .results
    })
    .unwrap();
    assert!(
        serde_json::from_value::<combat_fixtures::Fixture>(
